...
public class DcrlayoutService {
	...
	public String /*void*/ gerarArquivoTXT_Linux(Integer idMatriz, String partnumpd, String tpprd, String path) throws IOException, NoSuchFieldException, SecurityException, IllegalArgumentException, IllegalAccessException {
        		
		String fileName = tpprd+String.valueOf(idMatriz)+"_"+partnumpd.replace("00000", "").replace(" ", "")+".txt";
		//path= "/home/sv_hmlnpi/dcr";
		String file = (path+"//"+fileName).replace("/\s/g", "");
		FileWriter fw = new FileWriter(file); 
		BufferedWriter bw = new BufferedWriter(fw); 
		StringBuffer sb = null;
		Sort sort = Sort.by(Sort.Direction.ASC, "key.idreg", "posini");
		List<Dcrlayout> campos =  repository.findAll(sort);
		String representante= userService.getRepresentanteCert();



		//BEGIN-tst: 
		// bw.write(fileName+" --> TST...........2");
		// bw.close();		
		// //return fileName;
		// return file;
		//END-tst
