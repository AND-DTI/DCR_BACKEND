...
public class RegrasController {
	...
	public ResponseEntity<Object> createProcessamento(@RequestBody DcrregraDTO dto, HttpServletRequest request) {
		...
		try {
			...
			//DcrregraKeyDTO key = new DcrregraKeyDTO(Auxiliar.getDtFormated(), Auxiliar.getDtFormated()); j4 - not used
			...
			if(dcrOld.size() > 1 && dcrOld.get(dcrOld.size() - 2) != null) {
				...
				Dcrregra regraAtiva = dcr.get();
				service.save2(dto, regraAtiva, request);
		    }
			
			//Dcrregra dcrNew = service.create(dto, request); 4 - not used
