...
public class RegrasController {
	...
	public ResponseEntity<Object> createProcessamento(@RequestBody DcrregraDTO dto, HttpServletRequest request) {
		...
		try {
			...
			DcrregraKeyDTO key = new DcrregraKeyDTO(dto.confvigini(), dto.confvigfim());
			Optional<Dcrregra> dcr = service.getByDate(key);
			
			if (!dcr.isEmpty()) {
				return ResponseEntity.status(HttpStatus.BAD_REQUEST)
						.header("Accept", "application/json")
						.body("Regra com essa data já existe!");
		    }
	...
	@ResponseStatus(HttpStatus.OK)
	public ResponseEntity<Object> getByDate(@RequestBody DcrregraKeyDTO key, HttpServletRequest request) {
	...
	@Operation(summary = "Altera uma regra existente")
	@ApiResponses(value = {
	        @ApiResponse(responseCode = "201", description = "Regra atualizada!"),
	        @ApiResponse(responseCode = "500", description = "Error!")
	})
	@ResponseStatus(HttpStatus.CREATED)
	public ResponseEntity<Object> updateProcessamento(@RequestBody DcrregraDTO dto, HttpServletRequest request) {
		...
		try {
			...
			Dcrregra dcrNew = service.update(dto, dcr.get(), request);
			return ResponseEntity.status(HttpStatus.CREATED)
			        .header("Accept", "application/json")
			            .body(dcrNew);
