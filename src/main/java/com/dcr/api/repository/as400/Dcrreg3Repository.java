...
public interface Dcrreg3Repository extends JpaRepository<Dcrreg3, Dcrreg3Key>{
	@Query(value = "SELECT * FROM HD4DCDHH.DCRREG3 AS dcr WHERE dcr.idmatriz = :idmatriz AND dcr.partnumpd = :partnumpd AND dcr.tpprd = :tpprd ORDER BY dcr.numcomp", nativeQuery = true)
