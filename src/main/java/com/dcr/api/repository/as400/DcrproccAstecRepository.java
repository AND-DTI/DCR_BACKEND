...
public interface DcrproccAstecRepository extends JpaRepository<Dcrprocc, DcrproccKey>{
	...
	(IDMATRIZ, PARTNUMPD, TPPRD, IDREG, CNPJ, CPFRL, PPB, DENOM, NCM, UNDCOM, 
	 PESO, SALARIOS, ENCARGOS, TPDCRE, DCRANT, PROCRETIF, VRSPGD, ORIGDCR, TPCOEF, 
	...
	from 
		HD4DCDHH.MTASTEC as PRD                
		left join
		HD4DCDHH.DCRREGRA conf on conf.stsconfig= 1 left join
		HD4DCDHH.CADPPB ppb on ppb.partnumpd = prd.partnumpd
	where 
		prd.IDMATRIZ= :idmatriz and prd.PARTNUMPD= :partnumpd 
	""", nativeQuery = true)
	...
	select 
		prd.idmatriz, prd.partnumpd, 'PC', '1', 1, prd.descrfb, prd.preco, 
		substr(prd.partnumpd,1,15) codint,  
		'DCRBACKEND', CAST(:itaudusr as char(10)), CAST(:itaudhst as char(30)),
		VARCHAR_FORMAT(CURRENT TIMESTAMP, 'YYYYMMDD') dtatual,
		CHAR(TIME(CURRENT TIMESTAMP),JIS) hratual  
	from 
	...
	select 
		prd.idmatriz, prd.partnumpd, 'PC', ins.partnum, '2' reg,
		Rownumber() Over(Partition by prd.partnumpd) as numcomp,
	...
	from 
		...
		HD4DCDHH.MTASTEINS as INS on ins.IDMATRIZ= prd.IDMATRIZ
	...
	FROM 
		...
		(select UITMPAI, nac.partdesc descpai, nac.itmorg orig_pai, nac.emcomp emp_pai, nac.partnew, 
		 ...
		 from   HD4DCDHH.MTASTEINS nac join
				HD4DCDHH.DCRSTRU s on s.umodelo= :partnumpd and nac.PARTNUM = s.UITMPAI 
		...
		HD4DCDHH.DCRREG2 reg2 on reg2.IDMATRIZ= prd.IDMATRIZ and reg2.PARTNUMPD= prd.PARTNUMPD and reg2.partnum= conjunto.UITMPAI 
	WHERE 
		prd.IDMATRIZ= :idmatriz and prd.PARTNUMPD= :partnumpd and
		ins.ITMORG = '3'
	""", nativeQuery = true)
	...
	SELECT 
		prd.idmatriz, prd.partnumpd, 'PC', ins.partnum, '4' reg,
		Rownumber() Over(Partition by prd.partnumpd) + nvl(lastcomp,0) as numcomp,
		'S', 'S', 'N' as indii,
		case when NUMDOC3='' then NUMDOC else NUMDOC3 end as DI,
		case when NUMDOC3='' then ADICAO else ADICAO3 end as ADICAO,
		case when NUMDOC3='' then ITADICAO else ITADICAO3 end as ITADICAO, 
	...
	WHERE 
		...
		where reg3.IDMATRIZ= prd.IDMATRIZ  and reg3.partnum= ins.partnum
		)	
	""", nativeQuery = true)
